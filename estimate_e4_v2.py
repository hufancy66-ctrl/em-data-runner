...
OUT=Path('outputs_e4_r2'); WB='https://api.worldbank.org/v2'; Y0,Y1=1996,2021

def get(url,params=None,timeout=180,retries=4):
 last=None
 for k in range(retries):
  try:
   r=requests.get(url,params=params,timeout=timeout,headers={'User-Agent':'EM-E4-estimator/2.0'}); r.raise_for_status(); return r
  except Exception as e: last=e; time.sleep(1.5*(k+1))
 raise last

def countries():
 rows=get(WB+'/country',{'format':'json','per_page':500}).json()[1]; return {x['id'] for x in rows if x.get('region',{}).get('id')!='NA'}
...
def credit_panel():
 ...
 cc=countries(); d=pd.DataFrame([{'iso3':x.get('countryiso3code'),'year':int(x['date']),'credit':x.get('value')} for x in rows]); d=d[d.iso3.isin(cc)].dropna(); d['F_bank']=d.credit/100.; return d[['iso3','year','F_bank']]

agg=pd.read_csv(OUT/'E4_agg.csv').rename(columns={'ref_area':'iso3'}); agg['year']=pd.to_numeric(agg.year,errors='coerce'); agg=agg.dropna(subset=['iso3','year','s_fin']); agg['year']=agg.year.astype(int)
m=agg.merge(credit_panel(),on=['iso3','year'],how='inner').dropna().reset_index(drop=True); m.to_csv(OUT/'E4_estimation_sample.csv',index=False)
